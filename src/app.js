...
export const canvas = new window.p5(sketch => { // eslint-disable-line
  ...
  sketch.setup = () => {
    ...
    symptomsFilter.onclick = () => {
      RUN.filters.symptoms = !RUN.filters.symptoms
      document.getElementById('symptoms-count').classList.toggle('show', RUN.filters.symptoms)
      startBalls()
      resetValues()
    }

    alertedFilter.onclick = () => {
      RUN.filters.alerted = !RUN.filters.alerted
      document.getElementById('alerted-count').classList.toggle('show', RUN.filters.alerted)
      startBalls()
      resetValues()
    }
  }
